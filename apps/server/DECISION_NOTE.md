...
## Notable implementation detail: structured output on NIM

The AI SDK's `generateObject` (native `json_schema` structured output) is **not supported** by NIM's OpenAI-compatible endpoint — probing showed it errors with *"responseFormat is not supported"* and some models hang or return empty. Every role therefore uses **JSON-instruct**: the prompt demands a JSON object with an exact shape, the response is parsed and validated with zod, and the call retries once on parse failure (critic falls back to `accept` rather than failing the run). This was verified empirically before building.

## What I cut / scoped down

- **Self-critique/reflection pass** (re-read the report, trigger one more research pass on gaps): listed as optional in the brief; the critic already covers per-answer gaps, so I prioritized the required pipeline + UI.
- **Perfect citations**: citation numbers come from the model and can occasionally mismatch the source list order; the sources themselves are real and clickable, which the brief says is a "reasonable attempt."
- **Error handling / auth / polish**: explicitly not evaluated; the SSE endpoint returns the error message as an event and the CLI exits non-zero.
